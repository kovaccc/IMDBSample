import 'package:flutter/foundation.dart';

typedef K = Keys;

class Keys {
  const Keys();

  static const loginButton = Key("loginButton");
  static const qSplashLogo = Key("qSplashLogo");
  static const bottomNavigation = Key("bottomNavigation");
  static const popularMoviesPage = Key("popularMoviesPage");
  static const favouriteMoviesPage = Key("favouriteMoviesPage");
  static const favouriteMoviesTab = Key("favouriteMoviesTab");
  static const popularMoviesPagedListView = Key("popularMoviesPagedListView");
  static const movieDetailsPage = Key("movieDetailsPage");
  static const popularMoviesListView = Key("popularMoviesListView");
}
